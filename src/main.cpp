...
static const int64_t nTargetTimespan = 60 * 60;  // bitcoinlite - every 1 hour
unsigned int nTargetSpacing = 1 * 60; // bitcoinlite - 1 minute
static const int64_t nInterval = nTargetTimespan / nTargetSpacing;

static const int64_t nDiffChangeTarget = 1;

unsigned int nStakeMinAge = 8 * 60 * 60; // bitcoinlite - 8 hours
unsigned int nStakeMaxAge = -1; // bitcoinlite - unlimited
unsigned int nModifierInterval = 10 * 60; // bitcoinlite - time to elapse before new modifier is computed
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "23 July 2014 - Bitcoins prepare a second coming in India";
        CTransaction txNew;
        txNew.nTime = 1406136354;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
